...
def create_mixture_csv(args):
    ...
    #snr = np.random.normal(25, 10, len(speech_names)*magnification)
    snr = [-5, 0, 5, 10, 15, 20, 25, 30] * int(len(speech_names)*magnification/8.0)
    random.shuffle(snr)
    rs = np.random.RandomState(0)
    out_csv_path = os.path.join(workspace, "mixture_csvs", "%s_98hour_even.csv" % data_type)
    pp_data.create_folder(os.path.dirname(out_csv_path))
    cnt = 0
    f = open(out_csv_path, 'w')
    f.write("%s\t%s\t%s\t%s\t%s\n" % ("speech_name", "noise_name", "noise_onset", "noise_offset", "snr"))
    for speech_na in speech_names:
        ...
        for noise_na in selected_noise_names:
            ...
            f.write("%s\t%s\t%d\t%d\t%f\n" % (speech_na, noise_na, noise_onset, nosie_offset, snr[cnt]))
            if cnt % 100 == 0:
                print(cnt)
            cnt += 1
    f.close()
...
def calculate_mixture_features(args):
    ...
    mixture_csv_path = os.path.join(workspace, "mixture_csvs", "test_1hour_even.csv")
    ...
    for i1 in xrange(1, len(lis)):
        [speech_na, noise_na, noise_onset, noise_offset, snr] = lis[i1]
        ...
        out_bare_na = os.path.join("%s.%s.%s" % 
            (os.path.splitext(speech_na)[0], os.path.splitext(noise_na)[0], (str(int(snr)) + "db")))
        out_audio_path = os.path.join(workspace, "mixed_audios", "spectrogram", 
            data_type, "crn_mixdb", "%s.wav" % out_bare_na)
        create_folder(os.path.dirname(out_audio_path))
        write_audio(out_audio_path, mixed_audio, fs)
        # Extract spectrogram. 
        mixed_complx_x = calc_sp(mixed_audio, mode='complex', n_window = 320, n_overlap = 160)
        speech_x = calc_sp(speech_audio, mode='magnitude', n_window = 320, n_overlap = 160)
        noise_x = calc_sp(noise_audio, mode='magnitude', n_window = 320, n_overlap = 160)
        # Write out features. 
...
def calc_sp(audio, mode, n_window, n_overlap):
...
def pack_features(args):
    ...
    train_type = "IRM"
    # Load all features. 
    feat_dir = os.path.join(workspace, "features", "spectrogram", data_type, "chinese_mixdb")
    ...
    for na in names:
        ...
        if cnt%10000==0 and cnt!=0:
            ...
            out_path = os.path.join(workspace, "packed_features", "spectrogram", data_type, "mask_mixdb" , "chinese_data%d.h5"%cnt)
...
def pack_crn_features(args):
    ...
    t1 = time.time()
    # Load all features. 
    feat_dir = os.path.join(workspace, "features", "spectrogram", data_type, "crn_mixdb")
    names = os.listdir(feat_dir)
    tfrecords_train_filename = '/data00/wangjinchao/sednn-master/mixture2clean_dnn/workspace/tfrecords/train/crn_mixdb/data_office.tfrecords'
    ...
    for na in names:
        ...
        # Cut target spectrogram and take the center frame of each 3D segment. 
        speech_x_3d = mat_2d_to_3d(speech_x, agg_num=n_concat, hop=n_hop)
        for i in range(mixed_x_3d.shape[0]):
            mixed_input = mixed_x_3d[i].astype(np.float32).tostring()
            label = speech_x_3d[i].astype(np.float32).tostring()
...
def compute_scaler(args):
    ...
    # Load data. 
    t1 = time.time()
    hdf5_path = os.path.join(workspace, "packed_features", "spectrogram", data_type, "mixdb", "data.h5")

    tr_hdf5_dir = os.path.join(workspace, "packed_features", "spectrogram", "train", "mixdb")
    tr_hdf5_names = os.listdir(tr_hdf5_dir)
    print('start loading h5')
    with h5py.File(os.path.join(tr_hdf5_dir, tr_hdf5_names[0]), 'r') as hf:
        x = hf.get('x')     
        x = np.array(x) 
    for na in tr_hdf5_names[1:2]:
        with h5py.File(os.path.join(tr_hdf5_dir, na), 'r') as hf:
            x1 = hf.get('x')     
            x1 = np.array(x1)
    ...
    # Write out scaler. 
    out_path = os.path.join(workspace, "packed_features", "spectrogram", data_type, "mixdb", "scaler.p")
    create_folder(os.path.dirname(out_path))
...
def istft():
    audio_output = scipy.signal.istft(
                    spec,
                    fs = 16000,
                    window = ham_win,
                    nfft = 320,
